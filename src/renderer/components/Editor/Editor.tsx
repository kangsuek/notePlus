          ...
          if (start === end) {
            if (isShiftTab) {
              ...
              const currentLineStart =
                textBeforeCursor.length - (lines[lines.length - 1]?.length || 0);
            ...
            const firstLineStart =
              ...
              (linesBeforeSelection[linesBeforeSelection.length - 1]?.length || 0);
            ...
            const newValue =
              text.substring(0, firstLineStart) + newSelectedText + text.substring(lastLineEnd);
            setText(newValue);
      ...
      const renderHighlightedText = useCallback(() => {
        ...
        searchResults.forEach((result, idx) => {
          ...
          if (result.index > lastIndex) {
            parts.push(
              <span key={`text-${lastIndex}`}>{text.substring(lastIndex, result.index)}</span>
            );
          }
