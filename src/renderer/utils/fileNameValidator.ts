...
import { MAX_FILENAME_LENGTH, DEFAULT_FILENAME } from '@renderer/constants/ui';
 ...
 */
/**
 * 파일명에서 금지된 문자를 제거하는 헬퍼 함수
 */
function removeIllegalCharacters(fileName: string): string {
  return fileName.replace(/[/\\:*?"<>|]/g, '');
}

/**
 * 파일명의 앞뒤 점과 공백을 제거하는 헬퍼 함수
 */
function trimDotsAndSpaces(fileName: string): string {
...
export function sanitizeFileName(fileName: string): string {
  ...
  // 1단계: 공백 trim 및 금지 문자 제거
  let sanitized = removeIllegalCharacters(fileName.trim());
  ...
  // 3단계: 앞뒤 점과 공백 제거
  sanitized = trimDotsAndSpaces(sanitized);

  // 4단계: 길이 제한
  sanitized = truncateFileName(sanitized);

  // 5단계: 최종 검증 - 여전히 유효하지 않으면 기본값
